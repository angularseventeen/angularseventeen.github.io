...
cd ~/src/angular/angularseventeengithubio/;
...
echo ''  >> README.md 2>&1;
echo '# Angular seventeen'  >> README.md 2>&1;
echo ''  >> README.md 2>&1;

echo ''  >> README.md 2>&1;
echo 'This project is live at [https://angularseventeen.github.io](https://angularseventeen.github.io "seventeen!") thanks to Github.'  >> README.md 2>&1;
echo ''  >> README.md 2>&1;
...
export NODE_OPTIONS="--max-old-space-size=8000"; time yarn run ng build --configuration production --output-path docs --output-hashing none --stats-json true --subresource-integrity true --verbose true  >> README.md 2>&1;
echo "\`\`\`"  >> README.md 2>&1;
git add .;
